# DoR Contract Proposal: Migrate src/bitbucket-cloud-validator/ → cli/src/adapters/bitbucket/cloud/

**Story:** `artefacts/2026-04-16-engine-consolidation/stories/ec1.4-bitbucket-cloud-migration.md`
**Date:** 2026-04-17
...
A move-only refactor relocating the bitbucket-cloud-validator subcomponent and its test(s) from `src/` into the CLI's authoritative layout:

1. `git mv src/bitbucket-cloud-validator/** cli/src/adapters/bitbucket/cloud/**` — all 1 subfile (index.js).
2. `git mv tests/check-bitbucket-cloud.js` → `cli/tests/adapters/bitbucket/cloud/` — the 1 associated test file.
3. Rewrite imports inside the moved test file: any `require('…src/bitbucket-cloud-validator…')` or `import … from '…src/bitbucket-cloud-validator…'` → new path (or relative from the test's new location).
...
5. Add a new vitest file at `cli/tests/adapters/bitbucket/cloud/migration-invariants.test.ts` implementing the 5 unit/integration/NFR assertions from the test plan.

## What will NOT be built

- **Any behaviour change to bitbucket-cloud-validator.** Interfaces, return shapes, and external calls are preserved verbatim. A ticket to improve readability or modernise the code is a separate feature.
- **Rewriting the existing test(s)** (check-bitbucket-cloud.js). They move verbatim; only import paths change.
- **SKILL.md / standards content** that references bitbucket-cloud-validator. The CLI provides a stable external API shape; references to the subcomponent continue to read correctly.
...
| AC2 | Unit — vitest assertion: `fs.readdirSync('src/bitbucket-cloud-validator', { recursive: true })` filtered to .js/.ts has length 0. | unit |
...
- **Pre-existing test stability:** The test currently under tests/check-bitbucket-cloud.js run to a stable exit code and test-count at HEAD. If the test is flaky pre-migration, the "exactly match" AC5 invariant becomes unreliable — a pre-existing flake must be stabilised or captured in the AC1 snapshot with explicit acknowledgement.
...
**Files moved (git mv):**
- src/bitbucket-cloud-validator/index.js → cli/src/adapters/bitbucket/cloud/index.js
- tests/check-bitbucket-cloud.js → cli/tests/adapters/bitbucket/cloud/check-bitbucket-cloud.js

**Files modified:**
- `package.json` (root) — test script chain path update
- cli/tests/adapters/bitbucket/cloud/check-bitbucket-cloud.js — import rewrites inside moved test

**Files added:**
- `cli/tests/adapters/bitbucket/cloud/migration-invariants.test.ts` — new vitest file implementing the test plan's unit/integration/NFR assertions
...
- Runtime code paths in cli/src/adapters/bitbucket/cloud/ beyond import rewrites in the moved tests
