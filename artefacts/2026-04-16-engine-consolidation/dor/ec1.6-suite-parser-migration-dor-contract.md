# DoR Contract Proposal: Migrate src/suite-parser/ → cli/src/engine/suite/

**Story:** `artefacts/2026-04-16-engine-consolidation/stories/ec1.6-suite-parser-migration.md`
**Date:** 2026-04-17
...
A move-only refactor relocating the suite-parser subcomponent and its test(s) from `src/` into the CLI's authoritative layout:

1. `git mv src/suite-parser/** cli/src/engine/suite/**` — all 1 subfile (index.js).
2. `git mv .github/scripts/check-suite.js` → `cli/tests/engine/suite/` — the 1 associated test file.
3. Rewrite imports inside the moved test file: any `require('…src/suite-parser…')` or `import … from '…src/suite-parser…'` → new path (or relative from the test's new location).
...
5. Add a new vitest file at `cli/tests/engine/suite/migration-invariants.test.ts` implementing the 5 unit/integration/NFR assertions from the test plan.

## What will NOT be built

- **Any behaviour change to suite-parser.** Interfaces, return shapes, and external calls are preserved verbatim. A ticket to improve readability or modernise the code is a separate feature.
- **Rewriting the existing test(s)** (check-suite.js). They move verbatim; only import paths change.
- **SKILL.md / standards content** that references suite-parser. The CLI provides a stable external API shape; references to the subcomponent continue to read correctly.
...
| AC2 | Unit — vitest assertion: `fs.readdirSync('src/suite-parser', { recursive: true })` filtered to .js/.ts has length 0. | unit |
...
- **Pre-existing test stability:** The test currently under .github/scripts/check-suite.js run to a stable exit code and test-count at HEAD. If the test is flaky pre-migration, the "exactly match" AC5 invariant becomes unreliable — a pre-existing flake must be stabilised or captured in the AC1 snapshot with explicit acknowledgement.
...
**Files moved (git mv):**
- src/suite-parser/index.js → cli/src/engine/suite/index.js
- .github/scripts/check-suite.js → cli/tests/engine/suite/check-suite.js

**Files modified:**
- `package.json` (root) — test script chain path update
- cli/tests/engine/suite/check-suite.js — import rewrites inside moved test

**Files added:**
- `cli/tests/engine/suite/migration-invariants.test.ts` — new vitest file implementing the test plan's unit/integration/NFR assertions
...
- Runtime code paths in cli/src/engine/suite/ beyond import rewrites in the moved tests
