# DoR Contract Proposal: Migrate src/definition-skill/ → cli/src/engine/story/

**Story:** `artefacts/2026-04-16-engine-consolidation/stories/ec1.7-definition-migration.md`
**Date:** 2026-04-17
...
A move-only refactor relocating the definition-skill subcomponent and its test(s) from `src/` into the CLI's authoritative layout:

1. `git mv src/definition-skill/** cli/src/engine/story/**` — all 1 subfile (helpers.js).
2. `git mv tests/check-definition-skill.js, tests/check-definition-skill.test.js` → `cli/tests/engine/story/` — the 2 associated test files.
3. Rewrite imports inside the moved test files: any `require('…src/definition-skill…')` or `import … from '…src/definition-skill…'` → new path (or relative from the test's new location).
...
5. Add a new vitest file at `cli/tests/engine/story/migration-invariants.test.ts` implementing the 5 unit/integration/NFR assertions from the test plan.

## What will NOT be built

- **Any behaviour change to definition-skill.** Interfaces, return shapes, and external calls are preserved verbatim. A ticket to improve readability or modernise the code is a separate feature.
- **Rewriting the existing test(s)** (check-definition-skill.js, check-definition-skill.test.js). They move verbatim; only import paths change.
- **SKILL.md / standards content** that references definition-skill. The CLI provides a stable external API shape; references to the subcomponent continue to read correctly.
...
| AC2 | Unit — vitest assertion: `fs.readdirSync('src/definition-skill', { recursive: true })` filtered to .js/.ts has length 0. | unit |
...
- **Pre-existing test stability:** The tests currently under tests/check-definition-skill.js, tests/check-definition-skill.test.js run to a stable exit code and test-count at HEAD. If the test is flaky pre-migration, the "exactly match" AC5 invariant becomes unreliable — a pre-existing flake must be stabilised or captured in the AC1 snapshot with explicit acknowledgement.
...
**Files moved (git mv):**
- src/definition-skill/helpers.js → cli/src/engine/story/helpers.js
- tests/check-definition-skill.js → cli/tests/engine/story/check-definition-skill.js
- tests/check-definition-skill.test.js → cli/tests/engine/story/check-definition-skill.test.js

**Files modified:**
- `package.json` (root) — test script chain path update
- cli/tests/engine/story/check-definition-skill.js, cli/tests/engine/story/check-definition-skill.test.js — import rewrites inside moved tests

**Files added:**
- `cli/tests/engine/story/migration-invariants.test.ts` — new vitest file implementing the test plan's unit/integration/NFR assertions
...
- Runtime code paths in cli/src/engine/story/ beyond import rewrites in the moved tests
