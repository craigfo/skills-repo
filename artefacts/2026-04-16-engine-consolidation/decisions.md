...
---
**2026-04-17 | RISK-ACCEPT | DoR W4 batch acknowledgement (ec1.1–ec1.7, ec2.1, ec2.2, ec3.1)**
**Decision:** Accept DoR warning W4 ("Verification script reviewed by a domain expert") as acknowledged-but-unsatisfied for all 10 engine-consolidation stories. Solo-operator scale: the story author, the reviewer, the contributor, and the PR reviewer are the same person. W4's purpose — catching specification defects before the coding agent implements — is instead discharged by the operator's self-review of each verification script at PR time, immediately before marking the PR ready for review (or, since PRs on this branch are opened as drafts and reviewed solo, at the point of flipping draft → ready).
**Alternatives considered:** (a) block all 10 DoRs until a second reviewer signs off each verification script (infeasible at solo scale); (b) drop the W4 check from the skill until scale grows (too broad — rules out useful discipline when a collaborator does join); (c) RISK-ACCEPT with a named operator check at PR time (chosen).
**Rationale:** W4 is a scale-dependent control. Its value at N=2+ contributors is catching the silent "agent verified against wrong AC" failure mode before implementation starts. At N=1, the same person authors the AC, the script, and verifies the output — the failure mode reduces to author-slips-and-catches-own-slip, which a second pass at PR time is well-positioned to catch. The acknowledge-with-named-check approach keeps the discipline visible in the state file (W4 status is ACKNOWLEDGE, not PASS) so it re-surfaces for a human review the moment the context scales.
**Made by:** operator (pickup /test-plan → /definition-of-ready transition, 2026-04-17).
**Revisit trigger:** First contributor joins the repo AND opens a PR that touches verification-script content; at that point W4 flips from acknowledge-with-named-check to a second-reviewer-required gate.

---
**2026-04-17 | RISK-ACCEPT | /branch-setup baseline — pre-existing p3.14 RED failures acknowledged**
**Decision:** Accept the 7 failing tests in `tests/check-p3.14-concepts-docs.js` as a pre-existing RED baseline for all 10 engine-consolidation inner-loop branches (ec1.1–ec1.7, ec2.1, ec2.2, ec3.1). These failures check for content in `docs/concepts/README.md` (audience statement, three-categories, reading-order note, ONBOARDING link) that has not yet landed — p3.14 story is the TDD RED-baseline commit (9c4ec7d) for a Phase 3 concept-docs story whose implementation is not in scope for engine-consolidation. None of the failures touch pipeline-state, cli/, src/ subcomponents, CONTRIBUTING.md, or product/* — all orthogonal to engine-consolidation's scope.
**Alternatives considered:** (a) fix p3.14 before ec3.1 inner loop (out of scope for this feature — p3.14 is a separate Phase 3 story); (b) skip the p3.14 test invocation in ec3.1's branch test chain (introduces a configuration drift between branches); (c) acknowledge as pre-existing and proceed (chosen).
**Rationale:** Blocking engine-consolidation on an unrelated Phase 3 story would couple two features that /decisions Q1 already confirmed should progress independently. The AC5 pre/post-migration test invariant for ec1.x subcomponents and AC2 scanner union-equality for ec3.1 are unaffected by the p3.14 failures; those invariants read specific test files, not the global `npm test` exit code. /verify-completion at the end of each story's inner loop will re-confirm that the failing tests remain exactly the 7 known p3.14 tests — any new failure is a story-specific regression that blocks the PR.
**Made by:** operator (autonomous inner-loop start, 2026-04-17).
**Revisit trigger:** (a) If p3.14 concepts-docs lands and the 7 tests turn green, remove this entry; (b) if the failing set grows beyond these 7 specific test names, treat the new failures as story-specific regressions, not pre-existing.
