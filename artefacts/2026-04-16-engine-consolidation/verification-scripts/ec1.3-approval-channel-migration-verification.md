# AC Verification Script: Migrate src/approval-channel/ → cli/src/adapters/approval-channel/

**Story reference:** `artefacts/2026-04-16-engine-consolidation/stories/ec1.3-approval-channel-migration.md`
**Technical test plan:** `artefacts/2026-04-16-engine-consolidation/test-plans/ec1.3-approval-channel-migration-test-plan.md`
**Script version:** 1
...
2. Confirm you are on the migration branch for this story — e.g. `feature/ec1.3-approval-channel-migration` (or the engine-consolidation outer-loop branch if you're doing all 7 migrations on one branch). Run: `git branch --show-current`. Record the branch name here: _________
3. Confirm the branch is **before** any `git mv` has been run for `src/approval-channel/`. Run: `ls src/approval-channel/ | head -5`. You should see files (index.js). If `src/approval-channel/` is already empty or missing, you are on a post-migration branch — stop and rewind before running Scenario 1.
...
**Steps:**
1. At the repo root, run: `node tests/check-dor-approval.js`.
...
6. Transcribe the numbers above into the story's DoD artefact (`artefacts/2026-04-16-engine-consolidation/dod/ec1.3-approval-channel-migration-dod.md` when it is created), in a block titled `### pre-migration-tests`.
...
**Steps:**
1. After all `git mv` operations for this story are complete (`index.js`), run: `ls src/approval-channel/ 2>/dev/null || echo "directory does not exist"`
2. If the directory still exists, run: `find src/approval-channel/ -type f \( -name "*.js" -o -name "*.ts" \) | wc -l`. The count should be `0`.
3. If the directory is fully deleted, the check passes trivially.

**Expected outcome:**
> Either the directory `src/approval-channel/` does not exist, or it exists with zero `.js` / `.ts` files inside it. Any remaining `.js` or `.ts` file is a missed move — stop, move it, then re-run this scenario.
...
**Steps:**
1. Run: `ls cli/src/adapters/approval-channel/`. You should see `index.js`.
2. Run: `ls cli/tests/adapters/approval-channel/`. You should see `check-dor-approval.js` (the test file that moved with the code).
3. Open `cli/tests/adapters/approval-channel/check-dor-approval.js` in your editor. Search for `src/approval-channel` inside the file (Ctrl+F / Cmd+F). There should be **zero matches** — every import, require, or path string referencing the old source path has been rewritten to the new one (`cli/src/adapters/approval-channel/...` or a relative path from the test file).

**Expected outcome:**
> Every expected file is present at its new path, and the moved test file contains no leftover references to the old `src/approval-channel/` location.
...
2. Run the moved test script from the new path:
   - `node cli/tests/adapters/approval-channel/check-dor-approval.js`
   - Or, if the migration wires it into vitest: `cd cli && npx vitest run tests/adapters/approval-channel/`
...
**Steps:**
1. Run: `git log --follow --oneline cli/src/adapters/approval-channel/index.js | head -20`
2. Scan the output. You should see commits from **before** the migration commit — i.e. commits that existed when the file was still at `src/approval-channel/index.js`.
