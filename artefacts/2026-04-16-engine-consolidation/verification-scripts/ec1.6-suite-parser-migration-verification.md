# AC Verification Script: Migrate src/suite-parser/ → cli/src/engine/suite/

**Story reference:** `artefacts/2026-04-16-engine-consolidation/stories/ec1.6-suite-parser-migration.md`
**Technical test plan:** `artefacts/2026-04-16-engine-consolidation/test-plans/ec1.6-suite-parser-migration-test-plan.md`
**Script version:** 1
...
2. Confirm you are on the migration branch for this story — e.g. `feature/ec1.6-suite-parser-migration` (or the engine-consolidation outer-loop branch if you're doing all 7 migrations on one branch). Run: `git branch --show-current`. Record the branch name here: _________
3. Confirm the branch is **before** any `git mv` has been run for `src/suite-parser/`. Run: `ls src/suite-parser/ | head -5`. You should see files (index.js). If `src/suite-parser/` is already empty or missing, you are on a post-migration branch — stop and rewind before running Scenario 1.
...
**Steps:**
1. At the repo root, run: `node .github/scripts/check-suite.js`.
...
6. Transcribe the numbers above into the story's DoD artefact (`artefacts/2026-04-16-engine-consolidation/dod/ec1.6-suite-parser-migration-dod.md` when it is created), in a block titled `### pre-migration-tests`.
...
**Steps:**
1. After all `git mv` operations for this story are complete (`index.js`), run: `ls src/suite-parser/ 2>/dev/null || echo "directory does not exist"`
2. If the directory still exists, run: `find src/suite-parser/ -type f \( -name "*.js" -o -name "*.ts" \) | wc -l`. The count should be `0`.
3. If the directory is fully deleted, the check passes trivially.

**Expected outcome:**
> Either the directory `src/suite-parser/` does not exist, or it exists with zero `.js` / `.ts` files inside it. Any remaining `.js` or `.ts` file is a missed move — stop, move it, then re-run this scenario.
...
**Steps:**
1. Run: `ls cli/src/engine/suite/`. You should see `index.js`.
2. Run: `ls cli/tests/engine/suite/`. You should see `check-suite.js` (the test file that moved with the code).
3. Open `cli/tests/engine/suite/check-suite.js` in your editor. Search for `src/suite-parser` inside the file (Ctrl+F / Cmd+F). There should be **zero matches** — every import, require, or path string referencing the old source path has been rewritten to the new one (`cli/src/engine/suite/...` or a relative path from the test file).

**Expected outcome:**
> Every expected file is present at its new path, and the moved test file contains no leftover references to the old `src/suite-parser/` location.
...
2. Run the moved test script from the new path:
   - `node cli/tests/engine/suite/check-suite.js`
   - Or, if the migration wires it into vitest: `cd cli && npx vitest run tests/engine/suite/`
...
**Steps:**
1. Run: `git log --follow --oneline cli/src/engine/suite/index.js | head -20`
2. Scan the output. You should see commits from **before** the migration commit — i.e. commits that existed when the file was still at `src/suite-parser/index.js`.
