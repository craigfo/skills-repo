# AC Verification Script: Migrate src/improvement-agent/ → cli/src/agents/improvement-agent/

**Story reference:** `artefacts/2026-04-16-engine-consolidation/stories/ec1.2-improvement-agent-migration.md`
**Technical test plan:** `artefacts/2026-04-16-engine-consolidation/test-plans/ec1.2-improvement-agent-migration-test-plan.md`
**Script version:** 1
...
2. Confirm you are on the migration branch for this story — e.g. `feature/ec1.2-improvement-agent-migration` (or the engine-consolidation outer-loop branch if you're doing all 7 migrations on one branch). Run: `git branch --show-current`. Record the branch name here: _________
3. Confirm the branch is **before** any `git mv` has been run for `src/improvement-agent/`. Run: `ls src/improvement-agent/ | head -5`. You should see files (failure-detector.js, challenger.js, trace-interface.js). If `src/improvement-agent/` is already empty or missing, you are on a post-migration branch — stop and rewind before running Scenario 1.
...
**Steps:**
1. At the repo root, run each test in turn: `node tests/check-improvement-agent.js`, `node tests/check-challenger.js`, `node tests/failure-detector.test.js`, `node tests/failure-detector.integration.test.js`.
...
6. Transcribe the numbers above into the story's DoD artefact (`artefacts/2026-04-16-engine-consolidation/dod/ec1.2-improvement-agent-migration-dod.md` when it is created), in a block titled `### pre-migration-tests`.
...
**Steps:**
1. After all `git mv` operations for this story are complete (`failure-detector.js`, `challenger.js`, `trace-interface.js` — every subfile), run: `ls src/improvement-agent/ 2>/dev/null || echo "directory does not exist"`
2. If the directory still exists, run: `find src/improvement-agent/ -type f \( -name "*.js" -o -name "*.ts" \) | wc -l`. The count should be `0`.
3. If the directory is fully deleted, the check passes trivially.

**Expected outcome:**
> Either the directory `src/improvement-agent/` does not exist, or it exists with zero `.js` / `.ts` files inside it. Any remaining `.js` or `.ts` file is a missed move — stop, move it, then re-run this scenario.
...
**Steps:**
1. Run: `ls cli/src/agents/improvement-agent/`. You should see `failure-detector.js`, `challenger.js`, `trace-interface.js`.
2. Run: `ls cli/tests/agents/improvement-agent/`. You should see all 4 test files: `check-improvement-agent.js`, `check-challenger.js`, `failure-detector.test.js`, `failure-detector.integration.test.js`.
3. Open each moved test file under `cli/tests/agents/improvement-agent/` in your editor. Search for `src/improvement-agent` inside the file (Ctrl+F / Cmd+F). There should be **zero matches** — every import, require, or path string referencing the old source path has been rewritten to the new one (`cli/src/agents/improvement-agent/...` or a relative path from the test file).

**Expected outcome:**
> Every expected file is present at its new path, and all moved test files contain no leftover references to the old `src/improvement-agent/` location.
...
2. Run the moved test scripts from the new path:
   - `node cli/tests/agents/improvement-agent/check-improvement-agent.js`
   - `node cli/tests/agents/improvement-agent/check-challenger.js`
   - `node cli/tests/agents/improvement-agent/failure-detector.test.js`
   - `node cli/tests/agents/improvement-agent/failure-detector.integration.test.js`
   - Or, in one pass: `cd cli && npx vitest run tests/agents/improvement-agent/`
...
**Steps:**
1. Run: `git log --follow --oneline cli/src/agents/improvement-agent/failure-detector.js | head -20`
2. Scan the output. You should see commits from **before** the migration commit — i.e. commits that existed when the file was still at `src/improvement-agent/failure-detector.js`.
