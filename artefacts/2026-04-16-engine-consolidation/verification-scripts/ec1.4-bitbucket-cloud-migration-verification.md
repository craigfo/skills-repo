# AC Verification Script: Migrate src/bitbucket-cloud-validator/ → cli/src/adapters/bitbucket/cloud/

**Story reference:** `artefacts/2026-04-16-engine-consolidation/stories/ec1.4-bitbucket-cloud-migration.md`
**Technical test plan:** `artefacts/2026-04-16-engine-consolidation/test-plans/ec1.4-bitbucket-cloud-migration-test-plan.md`
**Script version:** 1
...
2. Confirm you are on the migration branch for this story — e.g. `feature/ec1.4-bitbucket-cloud-validator-migration` (or the engine-consolidation outer-loop branch if you're doing all 7 migrations on one branch). Run: `git branch --show-current`. Record the branch name here: _________
3. Confirm the branch is **before** any `git mv` has been run for `src/bitbucket-cloud-validator/`. Run: `ls src/bitbucket-cloud-validator/ | head -5`. You should see files (index.js). If `src/bitbucket-cloud-validator/` is already empty or missing, you are on a post-migration branch — stop and rewind before running Scenario 1.
...
**Steps:**
1. At the repo root, run: `node tests/check-bitbucket-cloud.js`.
...
4. For each test file, run: `grep -cE "^(function|const|let|var)\s+(test|check|assert)" <path>` — this gives an approximate count of test-like functions in the file. Record the count: _________
...
6. Transcribe the numbers above into the story's DoD artefact (`artefacts/2026-04-16-engine-consolidation/dod/ec1.4-bitbucket-cloud-migration-dod.md` when it is created), in a block titled `### pre-migration-tests`.

**Expected outcome:**
> You have recorded numbers transcribed into the DoD artefact: (a) exit code, (b) count of test-like functions, (c) count of assertion sites. These numbers form the baseline that Scenario 5 will compare against.
...
**Steps:**
1. After all `git mv` operations for this story are complete (`index.js`), run: `ls src/bitbucket-cloud-validator/ 2>/dev/null || echo "directory does not exist"`
2. If the directory still exists, run: `find src/bitbucket-cloud-validator/ -type f \( -name "*.js" -o -name "*.ts" \) | wc -l`. The count should be `0`.
3. If the directory is fully deleted, the check passes trivially.

**Expected outcome:**
> Either the directory `src/bitbucket-cloud-validator/` does not exist, or it exists with zero `.js` / `.ts` files inside it. Any remaining `.js` or `.ts` file is a missed move — stop, move it, then re-run this scenario.
...
**Steps:**
1. Run: `ls cli/src/adapters/bitbucket/cloud/`. You should see `index.js`.
2. Run: `ls cli/tests/adapters/bitbucket/cloud/`. You should see `check-bitbucket-cloud.js` (the test file that moved with the code).
3. Open `cli/tests/adapters/bitbucket/cloud/check-bitbucket-cloud.js` in your editor. Search for `src/bitbucket-cloud-validator` inside the file (Ctrl+F / Cmd+F). There should be **zero matches** — every import, require, or path string referencing the old source path has been rewritten to the new one (`cli/src/adapters/bitbucket/cloud/...` or a relative path from the test file).

**Expected outcome:**
> Every expected file is present at its new path, and the moved test file contains no leftover references to the old `src/bitbucket-cloud-validator/` location.
...
1. Run this grep, exactly as written:
   ...
   ```
   grep -rn "src/bitbucket-cloud-validator" . \
     --exclude-dir=.git \
...
2. Run the moved test script from the new path:
   - `node cli/tests/adapters/bitbucket/cloud/check-bitbucket-cloud.js`
   - Or, if the migration wires it into vitest: `cd cli && npx vitest run tests/adapters/bitbucket/cloud/`
3. Record the **exit code** (`echo $?` immediately after each run). Re-run the grep from Scenario 1 step 4 and 5 but against the new path to get the function and assertion counts.
...
**Steps:**
1. Run: `git log --follow --oneline cli/src/adapters/bitbucket/cloud/index.js | head -20`
2. Scan the output. You should see commits from **before** the migration commit — i.e. commits that existed when the file was still at `src/bitbucket-cloud-validator/index.js`.
