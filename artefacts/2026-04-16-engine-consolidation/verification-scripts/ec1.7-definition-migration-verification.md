# AC Verification Script: Migrate src/definition-skill/ → cli/src/engine/story/

**Story reference:** `artefacts/2026-04-16-engine-consolidation/stories/ec1.7-definition-migration.md`
**Technical test plan:** `artefacts/2026-04-16-engine-consolidation/test-plans/ec1.7-definition-migration-test-plan.md`
**Script version:** 1
...
2. Confirm you are on the migration branch for this story — e.g. `feature/ec1.7-definition-skill-migration` (or the engine-consolidation outer-loop branch if you're doing all 7 migrations on one branch). Run: `git branch --show-current`. Record the branch name here: _________
3. Confirm the branch is **before** any `git mv` has been run for `src/definition-skill/`. Run: `ls src/definition-skill/ | head -5`. You should see files (helpers.js). If `src/definition-skill/` is already empty or missing, you are on a post-migration branch — stop and rewind before running Scenario 1.
...
**Steps:**
1. At the repo root, run each test in turn: `node tests/check-definition-skill.js`, `node tests/check-definition-skill.test.js`.
...
6. Transcribe the numbers above into the story's DoD artefact (`artefacts/2026-04-16-engine-consolidation/dod/ec1.7-definition-migration-dod.md` when it is created), in a block titled `### pre-migration-tests`.
...
**Steps:**
1. After all `git mv` operations for this story are complete (`helpers.js`), run: `ls src/definition-skill/ 2>/dev/null || echo "directory does not exist"`
2. If the directory still exists, run: `find src/definition-skill/ -type f \( -name "*.js" -o -name "*.ts" \) | wc -l`. The count should be `0`.
3. If the directory is fully deleted, the check passes trivially.

**Expected outcome:**
> Either the directory `src/definition-skill/` does not exist, or it exists with zero `.js` / `.ts` files inside it. Any remaining `.js` or `.ts` file is a missed move — stop, move it, then re-run this scenario.
...
**Steps:**
1. Run: `ls cli/src/engine/story/`. You should see `helpers.js`.
2. Run: `ls cli/tests/engine/story/`. You should see all 2 test files: `check-definition-skill.js`, `check-definition-skill.test.js`.
3. Open each moved test file under `cli/tests/engine/story/` in your editor. Search for `src/definition-skill` inside the file (Ctrl+F / Cmd+F). There should be **zero matches** — every import, require, or path string referencing the old source path has been rewritten to the new one (`cli/src/engine/story/...` or a relative path from the test file).

**Expected outcome:**
> Every expected file is present at its new path, and all moved test files contain no leftover references to the old `src/definition-skill/` location.
...
2. Run the moved test scripts from the new path:
   - `node cli/tests/engine/story/check-definition-skill.js`
   - `node cli/tests/engine/story/check-definition-skill.test.js`
   - Or, in one pass: `cd cli && npx vitest run tests/engine/story/`
...
**Steps:**
1. Run: `git log --follow --oneline cli/src/engine/story/helpers.js | head -20`
2. Scan the output. You should see commits from **before** the migration commit — i.e. commits that existed when the file was still at `src/definition-skill/helpers.js`.
