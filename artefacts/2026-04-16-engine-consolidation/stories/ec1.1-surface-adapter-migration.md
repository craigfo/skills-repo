## Story: Migrate src/surface-adapter/ → cli/src/adapters/surface-adapter/
...
I want to **move `src/surface-adapter/` into `cli/src/adapters/surface-adapter/` with its tests co-located**,
...
**AC1:** Given the current HEAD of `develop` as the baseline, When the operator captures a test-count + pass-rate snapshot by running the tests that currently cover this subcomponent (`.github/scripts/check-surface-adapter.js (1,732 LOC)`), Then the snapshot is recorded in this story's DoD artefact in a `pre-migration-tests` block with: (a) numeric test count, (b) pass rate, (c) exact test names.

**AC2:** Given the pre-migration snapshot exists (AC1), When the operator moves `src/surface-adapter/` (including subfiles: index.js, resolver.js, adapters/{git-native,iac,saas-api,saas-gui,m365-admin,manual}.js) into `cli/src/adapters/surface-adapter/` via `git mv` (or equivalent preserving git history), Then every file appears at the new path and `src/surface-adapter/` contains 0 `.js` / `.ts` files.

**AC3:** Given AC2 completed, When the operator moves the accompanying test files (`.github/scripts/check-surface-adapter.js (1,732 LOC)`) into `cli/tests/adapters/surface-adapter/`, Then the test files appear at the new path, their original location is empty of these files, and any imports inside the moved tests referencing `src/surface-adapter/` are updated to the new path.

**AC4:** Given AC2+AC3 completed, When the operator runs a repo-wide grep for references to the old path (`grep -r "src/surface-adapter" . --exclude-dir=.git --exclude-dir=node_modules`), Then the only matches are in documentation / decisions.md / discovery.md / changelog narrative (not executable code or imports).
...
**Note:** Largest subcomponent (1,471 src LOC; 1,732 test LOC). Test file currently lives in .github/scripts/ — moves to cli/tests/ per Q3.
