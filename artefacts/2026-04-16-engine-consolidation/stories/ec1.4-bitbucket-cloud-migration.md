## Story: Migrate src/bitbucket-cloud-validator/ → cli/src/adapters/bitbucket/cloud/
...
I want to **move `src/bitbucket-cloud-validator/` into `cli/src/adapters/bitbucket/cloud/` with its tests co-located**,
...
**AC1:** Given the current HEAD of `develop` as the baseline, When the operator captures a test-count + pass-rate snapshot by running the tests that currently cover this subcomponent (`tests/check-bitbucket-cloud.js (319 LOC)`), Then the snapshot is recorded in this story's DoD artefact in a `pre-migration-tests` block with: (a) numeric test count, (b) pass rate, (c) exact test names.

**AC2:** Given the pre-migration snapshot exists (AC1), When the operator moves `src/bitbucket-cloud-validator/` (including subfiles: index.js) into `cli/src/adapters/bitbucket/cloud/` via `git mv` (or equivalent preserving git history), Then every file appears at the new path and `src/bitbucket-cloud-validator/` contains 0 `.js` / `.ts` files.

**AC3:** Given AC2 completed, When the operator moves the accompanying test files (`tests/check-bitbucket-cloud.js (319 LOC)`) into `cli/tests/adapters/bitbucket/cloud/`, Then the test files appear at the new path, their original location is empty of these files, and any imports inside the moved tests referencing `src/bitbucket-cloud-validator/` are updated to the new path.

**AC4:** Given AC2+AC3 completed, When the operator runs a repo-wide grep for references to the old path (`grep -r "src/bitbucket-cloud-validator" . --exclude-dir=.git --exclude-dir=node_modules`), Then the only matches are in documentation / decisions.md / discovery.md / changelog narrative (not executable code or imports).
...
**Note:** Paired with ec1.5 (dc-validator) under cli/src/adapters/bitbucket/. Branches can proceed independently.
