## Test Plan: Migrate src/bitbucket-cloud-validator/ → cli/src/adapters/bitbucket/cloud/

**Story reference:** `artefacts/2026-04-16-engine-consolidation/stories/ec1.4-bitbucket-cloud-migration.md`
**Epic reference:** `artefacts/2026-04-16-engine-consolidation/epics/epic-1-subcomponent-migrations.md`
**Review reference:** `artefacts/2026-04-16-engine-consolidation/review/ec1.4-bitbucket-cloud-migration-review-1.md` — PASS, 0 HIGH / 0 MEDIUM / 2 LOW.
...
Migration ACs verify a structural move, not runtime behaviour. Unit coverage is by node assertion script; integration coverage is the existing `check-bitbucket-cloud.js` test running against the moved code; AC6 (PR evidence) is verified manually since it's a git-platform artefact, not runnable code.
...
| AC2 | `src/bitbucket-cloud-validator/` contains 0 `.js` / `.ts` files after `git mv`.                                            | 1       | —           | —   | —                     | —        | 🟢 |
| AC3 | Test file moved to `cli/tests/adapters/bitbucket/cloud/`; imports rewritten.               | 2       | —           | —   | —                     | —        | 🟢 |
| AC4 | No executable code or imports reference the old path `src/bitbucket-cloud-validator`.                        | 1       | —           | —   | —                     | —        | 🟢 |
...
**Source:** Synthetic — no runtime data involved. "Data" here is the repository file tree itself (file paths, file contents, grep output, test count from running the existing test).
...
|-----|---------------------------------------------------------------------------------------------|----------------------------------------------|------------------|-------|
| AC1 | Test count, pass rate, exact test names from running `tests/check-bitbucket-cloud.js` at HEAD.    | Output of `node tests/check-bitbucket-cloud.js` before `git mv`. | None             | Transcribed into DoD artefact. |
| AC2 | File listing of `src/bitbucket-cloud-validator/`.                                                           | `git ls-files src/bitbucket-cloud-validator/ \| grep -E '\.(js\|ts)$'` post-move. | None             | Expected empty. |
| AC3 | File listing of `cli/src/adapters/bitbucket/cloud/` and `cli/tests/adapters/bitbucket/cloud/`.                                | `git ls-files` post-move.                  | None             | Expected to contain the moved files. |
| AC4 | Repo-wide grep for `src/bitbucket-cloud-validator`.                                     | `grep -rn "src/bitbucket-cloud-validator" . --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=artefacts`. | None             | Expected: matches only in docs / decisions.md / discovery.md, not executable code. |
| AC5 | Test count, pass rate from running `cli/tests/adapters/bitbucket/cloud/check-bitbucket-cloud.js` at new path.                  | Output of the moved test post-move. | None             | Compared against AC1 snapshot. |
...
Unit tests are implemented as a single node check script: `cli/tests/adapters/bitbucket/cloud/migration-invariants.test.ts` (or equivalent vitest file — exact filename decided at implementation). Each assertion block below is one failing test.

### Test: `src/bitbucket-cloud-validator/` is empty of executable code post-migration

- **Verifies:** AC2
- **Precondition:** `git mv` has completed for `src/bitbucket-cloud-validator/**` → `cli/src/adapters/bitbucket/cloud/**`.
- **Action:** `fs.readdirSync('src/bitbucket-cloud-validator', { recursive: true })` filtered to `.js` / `.ts` files.
- **Expected result:** Array length is exactly 0. Asserts: `expect(files).toHaveLength(0)`. Optional: `expect(fs.existsSync('src/bitbucket-cloud-validator')).toBe(false)` if the directory itself is deleted.
...
- **Action:** For each expected file (`index.js`), assert `fs.existsSync('cli/src/adapters/bitbucket/cloud/' + relativePath) === true`.
- **Expected result:** Every expected file is present.
- **Edge case:** No.

### Test: moved test file present at the new CLI test path and imports are rewritten

- **Verifies:** AC3 (half — test files + import rewrite)
- **Precondition:** `git mv` has completed.
- **Action:** (a) assert `fs.existsSync(`cli/tests/adapters/bitbucket/cloud/check-bitbucket-cloud.js`) === true`; (b) read the file and assert **no line** matches the regex `/\brequire\(['"][^'"]*src/bitbucket-cloud-validator/` or `/\bimport[^;]*from\s+['"][^'"]*src/bitbucket-cloud-validator/`.
- **Expected result:** File present at new path; no imports reference the old `src/bitbucket-cloud-validator` path.
- **Edge case:** No.

### Test: no executable code imports `src/bitbucket-cloud-validator`

- **Verifies:** AC4
- **Precondition:** `git mv` + import-rewrite has completed.
- **Action:** Repo-wide regex scan for `require\(['"][^'"]*src/bitbucket-cloud-validator/` and `import[^;]*from\s+['"][^'"]*src/bitbucket-cloud-validator/` across all `.js`, `.ts`, `.mjs`, `.cjs` files, excluding `node_modules`, `.git`, and `artefacts/`.
...
### Test: `check-bitbucket-cloud.js` pass count at new path matches pre-migration snapshot

- **Verifies:** AC5
- **Components involved:** the moved test file (`cli/tests/adapters/bitbucket/cloud/check-bitbucket-cloud.js`) + the moved source at `cli/src/adapters/bitbucket/cloud/**`.
...
- **Action:** Run the moved test script: `node cli/tests/adapters/bitbucket/cloud/check-bitbucket-cloud.js` (or `npx vitest run cli/tests/adapters/bitbucket/cloud/` if the migration wires it into vitest). Capture test count, pass rate, and test names.
...
- **NFR addressed:** Audit
- **Measurement method:** Run `git log --follow cli/src/adapters/bitbucket/cloud/index.js`. Assert the log contains at least one commit from **before** the migration commit (SHA precedes the migration SHA).
- **Pass threshold:** At least one pre-migration commit appears in the `--follow` history.
- **Tool:** Shell assertion within the migration-invariants test file, using `child_process.execSync('git log --follow --pretty=%H cli/src/adapters/bitbucket/cloud/index.js')`.
...
- **Runtime behaviour tests for bitbucket-cloud-validator logic.** The existing test file (`check-bitbucket-cloud.js`) already covers that; AC5 verifies it still runs at the new path without drift.
