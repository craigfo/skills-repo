## Test Plan: Migrate src/approval-channel/ → cli/src/adapters/approval-channel/
...
Migration ACs verify a structural move, not runtime behaviour. Unit coverage is by node assertion script; integration coverage is the existing `check-dor-approval.js` test running against the moved code; AC6 (PR evidence) is verified manually since it's a git-platform artefact, not runnable code.
...
| AC2 | `src/approval-channel/` contains 0 `.js` / `.ts` files after `git mv`.                                            | 1       | —           | —   | —                     | —        | 🟢 |
| AC3 | Test file moved to `cli/tests/adapters/approval-channel/`; imports rewritten.               | 2       | —           | —   | —                     | —        | 🟢 |
| AC4 | No executable code or imports reference the old path `src/approval-channel`.                        | 1       | —           | —   | —                     | —        | 🟢 |
...
|-----|---------------------------------------------------------------------------------------------|----------------------------------------------|------------------|-------|
| AC1 | Test count, pass rate, exact test names from running `tests/check-dor-approval.js` at HEAD.    | Output of `node tests/check-dor-approval.js` before `git mv`. | None             | Transcribed into DoD artefact. |
| AC2 | File listing of `src/approval-channel/`.                                                           | `git ls-files src/approval-channel/ \| grep -E '\.(js\|ts)$'` post-move. | None             | Expected empty. |
| AC3 | File listing of `cli/src/adapters/approval-channel/` and `cli/tests/adapters/approval-channel/`.                                | `git ls-files` post-move.                  | None             | Expected to contain the moved files. |
| AC4 | Repo-wide grep for `src/approval-channel`.                                     | `grep -rn "src/approval-channel" . --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=artefacts`. | None             | Expected: matches only in docs / decisions.md / discovery.md, not executable code. |
| AC5 | Test count, pass rate from running `cli/tests/adapters/approval-channel/check-dor-approval.js` at new path.                  | Output of the moved test post-move. | None             | Compared against AC1 snapshot. |
...
Unit tests are implemented as a single node check script: `cli/tests/adapters/approval-channel/migration-invariants.test.ts` (or equivalent vitest file — exact filename decided at implementation). Each assertion block below is one failing test.

### Test: `src/approval-channel/` is empty of executable code post-migration

- **Verifies:** AC2
- **Precondition:** `git mv` has completed for `src/approval-channel/**` → `cli/src/adapters/approval-channel/**`.
- **Action:** `fs.readdirSync('src/approval-channel', { recursive: true })` filtered to `.js` / `.ts` files.
- **Expected result:** Array length is exactly 0. Asserts: `expect(files).toHaveLength(0)`. Optional: `expect(fs.existsSync('src/approval-channel')).toBe(false)` if the directory itself is deleted.
...
- **Action:** For each expected file (`index.js`), assert `fs.existsSync('cli/src/adapters/approval-channel/' + relativePath) === true`.
...
- **Precondition:** `git mv` has completed.
- **Action:** (a) assert `fs.existsSync(`cli/tests/adapters/approval-channel/check-dor-approval.js`) === true`; (b) read the file and assert **no line** matches the regex `/\brequire\(['"][^'"]*src/approval-channel/` or `/\bimport[^;]*from\s+['"][^'"]*src/approval-channel/`.
- **Expected result:** File present at new path; no imports reference the old `src/approval-channel` path.
- **Edge case:** No.

### Test: no executable code imports `src/approval-channel`

- **Verifies:** AC4
- **Precondition:** `git mv` + import-rewrite has completed.
- **Action:** Repo-wide regex scan for `require\(['"][^'"]*src/approval-channel/` and `import[^;]*from\s+['"][^'"]*src/approval-channel/` across all `.js`, `.ts`, `.mjs`, `.cjs` files, excluding `node_modules`, `.git`, and `artefacts/`.
...
### Test: `check-dor-approval.js` pass count at new path matches pre-migration snapshot

- **Verifies:** AC5
- **Components involved:** the moved test file (`cli/tests/adapters/approval-channel/check-dor-approval.js`) + the moved source at `cli/src/adapters/approval-channel/**`.
...
- **Action:** Run the moved test script: `node cli/tests/adapters/approval-channel/check-dor-approval.js` (or `npx vitest run cli/tests/adapters/approval-channel/` if the migration wires it into vitest). Capture test count, pass rate, and test names.
...
- **NFR addressed:** Audit
- **Measurement method:** Run `git log --follow cli/src/adapters/approval-channel/index.js`. Assert the log contains at least one commit from **before** the migration commit (SHA precedes the migration SHA).
- **Pass threshold:** At least one pre-migration commit appears in the `--follow` history.
- **Tool:** Shell assertion within the migration-invariants test file, using `child_process.execSync('git log --follow --pretty=%H cli/src/adapters/approval-channel/index.js')`.
...
- **Runtime behaviour tests for approval-channel logic.** The existing test file (`check-dor-approval.js`) already covers that; AC5 verifies it still runs at the new path without drift.
