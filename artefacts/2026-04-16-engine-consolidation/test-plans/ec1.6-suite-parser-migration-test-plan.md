## Test Plan: Migrate src/suite-parser/ → cli/src/engine/suite/
...
**Review reference:** `artefacts/2026-04-16-engine-consolidation/review/ec1.6-suite-parser-migration-review-1.md` — PASS, 0 HIGH / 0 MEDIUM / 2 LOW.
...
Migration ACs verify a structural move, not runtime behaviour. Unit coverage is by node assertion script; integration coverage is the existing `check-suite.js` test running against the moved code; AC6 (PR evidence) is verified manually since it's a git-platform artefact, not runnable code.
...
| AC2 | `src/suite-parser/` contains 0 `.js` / `.ts` files after `git mv`.                                            | 1       | —           | —   | —                     | —        | 🟢 |
| AC3 | Test file moved to `cli/tests/engine/suite/`; imports rewritten.               | 2       | —           | —   | —                     | —        | 🟢 |
| AC4 | No executable code or imports reference the old path `src/suite-parser`.                        | 1       | —           | —   | —                     | —        | 🟢 |
...
|-----|---------------------------------------------------------------------------------------------|----------------------------------------------|------------------|-------|
| AC1 | Test count, pass rate, exact test names from running `.github/scripts/check-suite.js` at HEAD.    | Output of `node .github/scripts/check-suite.js` before `git mv`. | None             | Transcribed into DoD artefact. |
| AC2 | File listing of `src/suite-parser/`.                                                           | `git ls-files src/suite-parser/ \| grep -E '\.(js\|ts)$'` post-move. | None             | Expected empty. |
| AC3 | File listing of `cli/src/engine/suite/` and `cli/tests/engine/suite/`.                                | `git ls-files` post-move.                  | None             | Expected to contain the moved files. |
| AC4 | Repo-wide grep for `src/suite-parser`.                                     | `grep -rn "src/suite-parser" . --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=artefacts`. | None             | Expected: matches only in docs / decisions.md / discovery.md, not executable code. |
| AC5 | Test count, pass rate from running `cli/tests/engine/suite/check-suite.js` at new path.                  | Output of the moved test post-move. | None             | Compared against AC1 snapshot. |
...
Unit tests are implemented as a single node check script: `cli/tests/engine/suite/migration-invariants.test.ts` (or equivalent vitest file — exact filename decided at implementation). Each assertion block below is one failing test.

### Test: `src/suite-parser/` is empty of executable code post-migration

- **Verifies:** AC2
- **Precondition:** `git mv` has completed for `src/suite-parser/**` → `cli/src/engine/suite/**`.
- **Action:** `fs.readdirSync('src/suite-parser', { recursive: true })` filtered to `.js` / `.ts` files.
- **Expected result:** Array length is exactly 0. Asserts: `expect(files).toHaveLength(0)`. Optional: `expect(fs.existsSync('src/suite-parser')).toBe(false)` if the directory itself is deleted.
...
- **Action:** For each expected file (`index.js`), assert `fs.existsSync('cli/src/engine/suite/' + relativePath) === true`.
...
- **Precondition:** `git mv` has completed.
- **Action:** (a) assert `fs.existsSync(`cli/tests/engine/suite/check-suite.js`) === true`; (b) read the file and assert **no line** matches the regex `/\brequire\(['"][^'"]*src/suite-parser/` or `/\bimport[^;]*from\s+['"][^'"]*src/suite-parser/`.
- **Expected result:** File present at new path; no imports reference the old `src/suite-parser` path.
- **Edge case:** No.

### Test: no executable code imports `src/suite-parser`

- **Verifies:** AC4
- **Precondition:** `git mv` + import-rewrite has completed.
- **Action:** Repo-wide regex scan for `require\(['"][^'"]*src/suite-parser/` and `import[^;]*from\s+['"][^'"]*src/suite-parser/` across all `.js`, `.ts`, `.mjs`, `.cjs` files, excluding `node_modules`, `.git`, and `artefacts/`.
...
### Test: `check-suite.js` pass count at new path matches pre-migration snapshot

- **Verifies:** AC5
- **Components involved:** the moved test file (`cli/tests/engine/suite/check-suite.js`) + the moved source at `cli/src/engine/suite/**`.
...
- **Action:** Run the moved test script: `node cli/tests/engine/suite/check-suite.js` (or `npx vitest run cli/tests/engine/suite/` if the migration wires it into vitest). Capture test count, pass rate, and test names.
...
- **NFR addressed:** Audit
- **Measurement method:** Run `git log --follow cli/src/engine/suite/index.js`. Assert the log contains at least one commit from **before** the migration commit (SHA precedes the migration SHA).
- **Pass threshold:** At least one pre-migration commit appears in the `--follow` history.
- **Tool:** Shell assertion within the migration-invariants test file, using `child_process.execSync('git log --follow --pretty=%H cli/src/engine/suite/index.js')`.
...
- **Runtime behaviour tests for suite-parser logic.** The existing test file (`check-suite.js`) already covers that; AC5 verifies it still runs at the new path without drift.
