## Test Plan: Migrate src/definition-skill/ → cli/src/engine/story/

**Story reference:** `artefacts/2026-04-16-engine-consolidation/stories/ec1.7-definition-migration.md`
**Epic reference:** `artefacts/2026-04-16-engine-consolidation/epics/epic-1-subcomponent-migrations.md`
**Review reference:** `artefacts/2026-04-16-engine-consolidation/review/ec1.7-definition-migration-review-1.md` — PASS, 0 HIGH / 0 MEDIUM / 2 LOW.
...
Migration ACs verify a structural move, not runtime behaviour. Unit coverage is by node assertion script; integration coverage is the existing `check-definition-skill.js`, `check-definition-skill.test.js` tests running against the moved code; AC6 (PR evidence) is verified manually since it's a git-platform artefact, not runnable code.
...
| AC2 | `src/definition-skill/` contains 0 `.js` / `.ts` files after `git mv`.                                            | 1       | —           | —   | —                     | —        | 🟢 |
| AC3 | Test files moved to `cli/tests/engine/story/`; imports rewritten.               | 2       | —           | —   | —                     | —        | 🟢 |
| AC4 | No executable code or imports reference the old path `src/definition-skill`.                        | 1       | —           | —   | —                     | —        | 🟢 |
...
|-----|---------------------------------------------------------------------------------------------|----------------------------------------------|------------------|-------|
| AC1 | Test count, pass rate, exact test names from running `tests/check-definition-skill.js`, `tests/check-definition-skill.test.js` at HEAD.    | Output of `node tests/check-definition-skill.js` (and equivalent for other test files) before `git mv`. | None             | Transcribed into DoD artefact. |
| AC2 | File listing of `src/definition-skill/`.                                                           | `git ls-files src/definition-skill/ \| grep -E '\.(js\|ts)$'` post-move. | None             | Expected empty. |
| AC3 | File listing of `cli/src/engine/story/` and `cli/tests/engine/story/`.                                | `git ls-files` post-move.                  | None             | Expected to contain the moved files. |
| AC4 | Repo-wide grep for `src/definition-skill`.                                     | `grep -rn "src/definition-skill" . --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=artefacts`. | None             | Expected: matches only in docs / decisions.md / discovery.md, not executable code. |
| AC5 | Test count, pass rate from running `cli/tests/engine/story/check-definition-skill.js`, `cli/tests/engine/story/check-definition-skill.test.js` at new path.                  | Output of the moved tests post-move. | None             | Compared against AC1 snapshot. |
...
Unit tests are implemented as a single node check script: `cli/tests/engine/story/migration-invariants.test.ts` (or equivalent vitest file — exact filename decided at implementation). Each assertion block below is one failing test.

### Test: `src/definition-skill/` is empty of executable code post-migration

- **Verifies:** AC2
- **Precondition:** `git mv` has completed for `src/definition-skill/**` → `cli/src/engine/story/**`.
- **Action:** `fs.readdirSync('src/definition-skill', { recursive: true })` filtered to `.js` / `.ts` files.
- **Expected result:** Array length is exactly 0. Asserts: `expect(files).toHaveLength(0)`. Optional: `expect(fs.existsSync('src/definition-skill')).toBe(false)` if the directory itself is deleted.
...
- **Action:** For each expected file (`helpers.js`), assert `fs.existsSync('cli/src/engine/story/' + relativePath) === true`.
...
- **Precondition:** `git mv` has completed.
- **Action:** (a) assert `fs.existsSync(`cli/tests/engine/story/check-definition-skill.js`) === true` and `fs.existsSync(`cli/tests/engine/story/check-definition-skill.test.js`) === true`; (b) read each file and assert **no line** matches the regex `/\brequire\(['"][^'"]*src/definition-skill/` or `/\bimport[^;]*from\s+['"][^'"]*src/definition-skill/`.
- **Expected result:** Files present at new path; no imports reference the old `src/definition-skill` path.
- **Edge case:** No.

### Test: no executable code imports `src/definition-skill`

- **Verifies:** AC4
- **Precondition:** `git mv` + import-rewrite has completed.
- **Action:** Repo-wide regex scan for `require\(['"][^'"]*src/definition-skill/` and `import[^;]*from\s+['"][^'"]*src/definition-skill/` across all `.js`, `.ts`, `.mjs`, `.cjs` files, excluding `node_modules`, `.git`, and `artefacts/`.
...
### Test: `check-definition-skill.js`, `check-definition-skill.test.js` pass count at new path matches pre-migration snapshot

- **Verifies:** AC5
- **Components involved:** the moved test files (`cli/tests/engine/story/check-definition-skill.js`, `cli/tests/engine/story/check-definition-skill.test.js`) + the moved source at `cli/src/engine/story/**`.
...
- **Action:** Run each moved test file (`cli/tests/engine/story/check-definition-skill.js`, `cli/tests/engine/story/check-definition-skill.test.js`) from the new path, or `npx vitest run cli/tests/engine/story/` to cover them in one pass. Capture test count, pass rate, and test names.
...
- **NFR addressed:** Audit
- **Measurement method:** Run `git log --follow cli/src/engine/story/helpers.js`. Assert the log contains at least one commit from **before** the migration commit (SHA precedes the migration SHA).
- **Pass threshold:** At least one pre-migration commit appears in the `--follow` history.
- **Tool:** Shell assertion within the migration-invariants test file, using `child_process.execSync('git log --follow --pretty=%H cli/src/engine/story/helpers.js')`.
...
- **Runtime behaviour tests for definition-skill logic.** The existing test files (`check-definition-skill.js`, `check-definition-skill.test.js`) already cover that; AC5 verifies they still run at the new path without drift.
